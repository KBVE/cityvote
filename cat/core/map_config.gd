...
const CHUNKS_WIDE: int = 32  # MAP_WIDTH / CHUNK_SIZE = 1024 / 32 = 32 chunks
const CHUNKS_TALL: int = 32  # MAP_HEIGHT / CHUNK_SIZE = 1024 / 32 = 32 chunks
const TOTAL_CHUNKS: int = CHUNKS_WIDE * CHUNKS_TALL  # 32 * 32 = 1024 chunks
...
static func tile_to_chunk(tile_coords: Vector2i) -> Vector2i:
	return Vector2i(int(tile_coords.x / CHUNK_SIZE), int(tile_coords.y / CHUNK_SIZE))
...
static func chunk_index_to_coords(chunk_index: int) -> Vector2i:
	return Vector2i(chunk_index % CHUNKS_WIDE, int(chunk_index / CHUNKS_WIDE))
