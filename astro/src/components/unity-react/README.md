...
useEffect(() => {
  ...
  const unsubHeightmap = unityService.onArrayData('TerrainData_Heightmap', (event: UnityArrayDataEvent) => {
  ...
  const unsubMesh = unityService.onArrayData('MeshGenerated_Vertices', (event) => {
    if (event.dataType === 'Float32Array') {
      const vertices = event.data as Float32Array;
      console.log('Received mesh with', vertices.length / 3, 'vertices');
  ...
  const unsubTriangles = unityService.onArrayData('MeshGenerated_Triangles', (event) => {
