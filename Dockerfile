...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    --mount=type=cache,target=/usr/local/cargo/git \
    cargo build --release && \
    strip target/release/kbve-cityvote
...
COPY --from=builder /app/target/release/kbve-cityvote /app/kbve-cityvote
